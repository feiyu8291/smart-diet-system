...
DROP TABLE IF EXISTS diet_dish_cooking_branch;
DROP TABLE IF EXISTS diet_dish_like;
DROP TABLE IF EXISTS diet_dish_collect;
DROP TABLE IF EXISTS diet_meal_feedback;
...
COMMENT ON COLUMN diet_user_health_profile.member_birthday IS '成员出生日期';
...
COMMENT ON COLUMN diet_dish_ingredient.branch_id IS '关联做法分支ID';
...
(
    ...
    fire_power       SMALLINT  NOT NULL DEFAULT 0,
    del_flag         SMALLINT  NOT NULL DEFAULT 0,
    create_by        VARCHAR(64),
    update_by        VARCHAR(64),
    create_time      TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
...
COMMENT ON COLUMN diet_dish_step_relation.branch_id IS '关联做法分支ID';
...
COMMENT ON COLUMN diet_dish_step_relation.duration_seconds IS '该步骤推荐烹饪秒数，默认0';
COMMENT ON COLUMN diet_dish_step_relation.fire_power IS '推荐火候(0非热处理 1小火/慢炖 2中火/清蒸 3大火/爆炒)';
...
COMMENT ON COLUMN diet_family_meal_portion.branch_id IS '就餐做法分支ID';
...
/*==============================================================*/
/* Table: diet_dish_cooking_branch                              */
/*==============================================================*/
CREATE TABLE IF NOT EXISTS diet_dish_cooking_branch
(
...
COMMENT ON TABLE diet_dish_cooking_branch IS '业务-菜品烹饪做法分支表';
COMMENT ON COLUMN diet_dish_cooking_branch.branch_id IS '主键做法分支ID';
COMMENT ON COLUMN diet_dish_cooking_branch.dish_id IS '关联主菜品ID';
COMMENT ON COLUMN diet_dish_cooking_branch.branch_name IS '做法分支名称(如：无糖少油版)';
COMMENT ON COLUMN diet_dish_cooking_branch.cuisine_type IS '做法专属菜系类型';
COMMENT ON COLUMN diet_dish_cooking_branch.diet_mode IS '建议就餐膳食模式(0-正常饮食, 1-轻食减脂, 2-放纵餐)';
COMMENT ON COLUMN diet_dish_cooking_branch.creator_user_id IS '做法提交创作者ID(关联sys_user.user_id)';
COMMENT ON COLUMN diet_dish_cooking_branch.cover_image_id IS '做法分支封面图片ID(关联sys_file_storage.storage_id)';
COMMENT ON COLUMN diet_dish_cooking_branch.calories IS '每100g合算热量(kcal)';
COMMENT ON COLUMN diet_dish_cooking_branch.protein IS '每100g合算蛋白质(g)';
COMMENT ON COLUMN diet_dish_cooking_branch.fat IS '每100g合算脂肪(g)';
COMMENT ON COLUMN diet_dish_cooking_branch.carbs IS '每100g合算碳水(g)';

/*==============================================================*/
/* Table: diet_dish_like                                         */
/*==============================================================*/
CREATE TABLE IF NOT EXISTS diet_dish_like
(
    ...
    create_time TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    PRIMARY KEY (like_id),
    CONSTRAINT uniq_user_branch_like UNIQUE (user_id, branch_id)
);

COMMENT ON TABLE diet_dish_like IS '业务-做法分支点赞表';
COMMENT ON COLUMN diet_dish_like.like_id IS '主键ID';
COMMENT ON COLUMN diet_dish_like.user_id IS '点赞用户ID(关联sys_user.user_id)';
COMMENT ON COLUMN diet_dish_like.branch_id IS '关联做法分支ID';

/*==============================================================*/
/* Table: diet_dish_collect                                      */
/*==============================================================*/
CREATE TABLE IF NOT EXISTS diet_dish_collect
(
    ...
    CONSTRAINT uniq_user_branch_collect UNIQUE (user_id, branch_id)
);

COMMENT ON TABLE diet_dish_collect IS '业务-做法分支收藏表';
COMMENT ON COLUMN diet_dish_collect.collect_id IS '主键ID';
COMMENT ON COLUMN diet_dish_collect.user_id IS '收藏用户ID(关联sys_user.user_id)';
COMMENT ON COLUMN diet_dish_collect.branch_id IS '关联做法分支ID';

/*==============================================================*/
/* Table: diet_meal_feedback                                    */
/*==============================================================*/
CREATE TABLE IF NOT EXISTS diet_meal_feedback
(
    feedback_id  BIGSERIAL NOT NULL,
    meal_plan_id BIGINT    NOT NULL,
    profile_id   BIGINT    NOT NULL,
    eat_status   SMALLINT  NOT NULL,
    skip_reason  VARCHAR(100),
    skip_note    VARCHAR(255),
    create_time  TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    PRIMARY KEY (feedback_id)
);

COMMENT ON TABLE diet_meal_feedback IS '业务-用餐就餐打卡反馈表';
COMMENT ON COLUMN diet_meal_feedback.feedback_id IS '主键ID';
COMMENT ON COLUMN diet_meal_feedback.meal_plan_id IS '关联食谱膳食计划ID';
COMMENT ON COLUMN diet_meal_feedback.profile_id IS '就餐人档案ID';
COMMENT ON COLUMN diet_meal_feedback.eat_status IS '用餐状态(1-已餐, 2-未餐)';
COMMENT ON COLUMN diet_meal_feedback.skip_reason IS '未餐原因(外出/加班/生病/没有胃口等)';
COMMENT ON COLUMN diet_meal_feedback.skip_note IS '未餐详细备注';
