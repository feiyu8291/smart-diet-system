...
-- 插入一级及二级菜单
INSERT INTO sys_menu (menu_id, parent_id, menu_name, menu_code, request_url, menu_type, menu_icon, sort_order, menu_description, create_by)
...
-- 用户管理 按钮与接口
       ...
       (11, 2, '重置密码', 'system/user/resetPassword', '/sys/user/resetPassword', 2, NULL, NULL, '用户管理-重置密码', '系统管理员'),
       (12, 2, '启用/禁用', 'system/user/updateStatus', '/sys/user/updateStatus', 2, NULL, NULL, '用户管理-状态修改', '系统管理员'),
       (13, 2, '分配角色', 'system/user/saveRoles', '/sys/user/saveRoles', 2, NULL, NULL, '用户管理-角色分配', '系统管理员'),
...
-- 角色管理 按钮与接口
       ...
       (20, 4, '角色配置', 'system/role/configMenus', '/sys/role/configMenus', 2, NULL, NULL, '角色管理-配置角色', '系统管理员'),
...
-- 字典管理 按钮与接口
       ...
       (24, 5, '字典刷新缓存', 'system/dict/refreshCache', '/sys/dict/refreshCache', 2, NULL, NULL, '数据字典-刷新缓存', '系统管理员'),

-- 定时任务 按钮与接口
       (25, 6, '任务新增', 'system/quartz/save', '/sys/quartzJob/save', 2, NULL, NULL, '定时任务-新增', '系统管理员'),
       (26, 6, '任务修改', 'system/quartz/update', '/sys/quartzJob/update', 2, NULL, NULL, '定时任务-修改', '系统管理员'),
       (27, 6, '任务删除', 'system/quartz/delete', '/sys/quartzJob/delete', 2, NULL, NULL, '定时任务-删除', '系统管理员'),
       (28, 6, '暂停/恢复任务', 'system/quartz/updateStatus', '/sys/quartzJob/updateStatus', 2, NULL, NULL, '定时任务-状态切换', '系统管理员'),
       (29, 6, '立即执行任务', 'system/quartz/runOnce', '/sys/quartzJob/runOnce', 2, NULL, NULL, '定时任务-立即执行', '系统管理员'),

-- 文件存储 按钮与接口
       (30, 7, '文件上传', 'system/file/upload', '/sys/fileStorage/upload', 2, NULL, NULL, '文件存储-文件上传', '系统管理员'),
       (31, 7, '文件删除', 'system/file/delete', '/sys/fileStorage/delete', 2, NULL, NULL, '文件存储-文件删除', '系统管理员'),
       (32, 7, '文件下载/预览', 'system/file/preview', '/sys/fileStorage/preview', 2, NULL, NULL, '文件存储-预览与下载', '系统管理员'),

-- 日志管理 按钮与接口
       (200, NULL, '日志管理', 'log_manage', 'log_manage', 0, 'Memo', 5, '系统日志管理根菜单', '系统管理员'),
       (201, 200, '登录日志', 'system/loginLog', '/sys/log/login/page', 1, 'document', 1, '用户系统登录历史', '系统管理员'),
       (202, 200, '操作日志', 'system/opLog', '/sys/log/operation/page', 1, 'document', 2, '核心业务流转轨迹审计', '系统管理员');
...
VALUES
    ...
    -- 未餐反馈原因类型
    ('未餐反馈原因', 'skip_reason_type', '1', '工作加班', '', '', '未餐原因-加班', 1, 0, 1, 1, '系统管理员'),
    ('未餐反馈原因', 'skip_reason_type', '2', '外出就餐', '', '', '未餐原因-外出', 1, 0, 1, 2, '系统管理员'),
    ('未餐反馈原因', 'skip_reason_type', '3', '身体不适', '', '', '未餐原因-生病', 1, 0, 1, 3, '系统管理员'),
    ('未餐反馈原因', 'skip_reason_type', '4', '胃口欠佳', '', '', '未餐原因-胃口差', 1, 0, 1, 4, '系统管理员'),
    ('未餐反馈原因', 'skip_reason_type', '5', '其他原因', '', '', '未餐原因-其他', 1, 0, 1, 5, '系统管理员'),

    -- 火候类型
    ('火候类型', 'fire_power_type', '0', '非热处理', '', '', '火候-非热处理', 1, 1, 1, 1, '系统管理员'),
    ('火候类型', 'fire_power_type', '1', '小火/慢炖', '', '', '火候-小火/慢炖', 1, 0, 1, 2, '系统管理员'),
    ('火候类型', 'fire_power_type', '2', '中火/清蒸', '', '', '火候-中火/清蒸', 1, 0, 1, 3, '系统管理员'),
    ('火候类型', 'fire_power_type', '3', '大火/爆炒', '', '', '火候-大火/爆炒', 1, 0, 1, 4, '系统管理员'),

    -- 菜系类型
    ('菜系类型', 'cuisine_type', 'chuan', '川菜', '', '', '八大菜系-川菜', 1, 0, 1, 1, '系统管理员'),
    ...
    -- 膳食模式类型
    ('膳食模式', 'diet_mode_type', '0', '正常饮食', '', '', '膳食模式-正常', 1, 1, 1, 1, '系统管理员'),
    ('膳食模式', 'diet_mode_type', '1', '轻食减脂', '', '', '膳食模式-轻食', 1, 0, 1, 2, '系统管理员'),
    ('膳食模式', 'diet_mode_type', '2', '放纵餐', '', '', '膳食模式-放纵', 1, 0, 1, 3, '系统管理员'),

    -- 食材类型
    ('食材类型', 'ingredient_type', '1', '荤菜类', '', '', '食材分类-荤菜', 1, 1, 1, 1, '系统管理员'),
    ('食材类型', 'ingredient_type', '2', '素菜类', '', '', '食材分类-素菜', 1, 0, 1, 2, '系统管理员'),
    ('食材类型', 'ingredient_type', '3', '调辅配料', '', '', '食材分类-调辅配料', 1, 0, 1, 3, '系统管理员'),
    ('食材类型', 'ingredient_type', '4', '基础调味', '', '', '食材分类-基础调味', 1, 0, 1, 4, '系统管理员');
